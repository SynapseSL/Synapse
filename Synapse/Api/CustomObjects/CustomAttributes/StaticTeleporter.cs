...
namespace Synapse.Api.CustomObjects.CustomAttributes
{
    public class StaticTeleporter : BasicTeleporter
    {
        ...
        {
            if (args.Count == 2 && Single.TryParse(args.At(1), out var result))
                return new Vector3(result, result, result);
            if (args.Count >= 4 && Single.TryParse(args.At(1), out var x) && Single.TryParse(args.At(2), out var y) && Single.TryParse(args.At(3), out var z))
