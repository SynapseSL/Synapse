    ...
    internal static class PlayerChangeItemPatch
    {
        ...
        private static bool SelectItem(Inventory __instance, ushort itemSerial)
        {
            try
            {
                ...
                var flag = (__instance.UserInventory.Items.TryGetValue(__instance.CurItem.SerialNumber, out var oldItem) && __instance.CurInstance != null) || __instance.CurItem.SerialNumber == 0;
                if (__instance.UserInventory.Items.TryGetValue(itemSerial, out var newItem) || itemSerial == 0)
                {
                    if (__instance.CurItem.SerialNumber > 0 && flag && !oldItem.CanHolster())
                        return false;

                    if (itemSerial != 0 && !newItem.CanEquip())
                        return false;
