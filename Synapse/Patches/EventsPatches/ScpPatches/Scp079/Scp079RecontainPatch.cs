...
namespace Synapse.Patches.EventsPatches.ScpPatches.Scp079
{
	...
	internal class EndOverchargePatch
	{
		...
		private static void EndOvercharge()
		{
			...
			{
				Api.Logger.Get.Error($"Synapse-Event: Scp079RecontainEvent Finished failed!!\n{e}");
			}
		}
	}
		...
		private static bool OnActivator(Recontainer079 __instance)
		{
			try
			{
				...
				else
				{
					...
					__instance._activatorButton.transform.localPosition = Vector3.Lerp(__instance._activatorButton.transform.localPosition,
						__instance._activatorPos, __instance._activatorLerpSpeed * Time.deltaTime);
			...
			{
				Api.Logger.Get.Error($"Synapse-Event: Scp079RecontainEvent Activator failed!!\n{e}");
				return true;
			}
		}
	}
