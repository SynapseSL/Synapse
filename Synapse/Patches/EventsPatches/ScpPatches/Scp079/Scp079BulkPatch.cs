    ...
    internal static class Scp079BulkPatch
    {
        ...
        private static bool Scp079Interact(Scp079PlayerScript __instance, Command079 command, string args, GameObject target)
        {
            ...
            try
            {
                ...
                var gotDoorVariant = target?.TryGetComponent(out doorVariant) ?? false;
                var list = GameCore.ConfigFile.ServerConfig.GetStringList("scp079_door_blacklist") ?? new List<string>();
                ...
                switch (command)
                {
                    case Command079.Door:
                        {
                            ...
                            var text = doorVariant.RequiredPermissions.RequiredPermissions.ToString();
                            var manaFromLabel = __instance.GetManaFromLabel("Door Interaction " + (text.Contains(",") ? text.Split(',')[0] : text), __instance.abilities);
                            ...
                            switch (actualResult)
                            {
                                case Scp079EventMisc.InteractionResult.Allow:
                                    {
                                        var targetState = doorVariant.TargetState;
                    ...
                    case Command079.Doorlock:
                        {
                            ...
                            var manaFromLabel = __instance.GetManaFromLabel("Door Lock Minimum", __instance.abilities);
                            ...
                            var intendedResult = action == Scp079EventMisc.DoorAction.Unlocking
                                ? __instance.lockedDoors.Contains(doorVariant.netId) ? Scp079EventMisc.InteractionResult.Allow : Scp079EventMisc.InteractionResult.Disallow
                                : manaFromLabel <= __instance.Mana ? Scp079EventMisc.InteractionResult.Allow : Scp079EventMisc.InteractionResult.NoEnergy;
                            SynapseController.Server.Events.Scp.Scp079.Invoke079DoorInteract(
                                __instance.gameObject.GetPlayer(),
                                action,
                                intendedResult,
                                ...
                                case Scp079EventMisc.InteractionResult.Allow when action == Scp079EventMisc.DoorAction.Unlocking:
                                    {
                                        ...
                                        _ = __instance.lockedDoors.Remove(doorVariant.netId);
                    ...
                    case Command079.Speaker:
                        {
                            var speakerQualifiedName = __instance.CurrentRoom.transform.parent.name + "/" + __instance.CurrentRoom.name + "/Scp079Speaker";
                            var speaker = GameObject.Find(speakerQualifiedName);
                            var manaFromLabel = __instance.GetManaFromLabel("Speaker Start", __instance.abilities);

                            var intendedResult = speaker is null
                                ? Scp079EventMisc.InteractionResult.Disallow
                                : manaFromLabel * 1.5f <= __instance.Mana ? Scp079EventMisc.InteractionResult.Allow : Scp079EventMisc.InteractionResult.NoEnergy;
                    ...
                    case Command079.ElevatorTeleport:
                        {
                            ...
                            var manaFromLabel = __instance.GetManaFromLabel("Elevator Teleport", __instance.abilities);
                            ...
                            foreach (var scp079Interactable in __instance.nearbyInteractables)
                            {
                            ...
                            if (ConsoleDebugMode.CheckImportance("SCP079", MessageImportance.LeastImportant, out var color))
                            {
                                Scp079Interactable scp079Interactable2 = null;
                                var dictionary = new Dictionary<Scp079Interactable.InteractableType, byte>();
                                foreach (var scp079Interactable3 in __instance.nearbyInteractables)
                                {
                                    if (dictionary.ContainsKey(scp079Interactable3.type))
                                    {
                                        var dictionary2 = dictionary;
                                        var type = scp079Interactable3.type;
                                        var b = dictionary2[type];
                                ...
                                if (scp079Interactable2 is null)
                                {
                                    ...
                                    using (var enumerator2 = dictionary.GetEnumerator())
                                    {
                                        while (enumerator2.MoveNext())
                                        {
                                            var keyValuePair = enumerator2.Current;
                                ...
                                {
                                    var str = "";
                                    var transform = scp079Interactable2.optionalObject.transform;
                                    for (var i = 0; i < 5; i++)
                                    {
                    ...
                    case Command079.ElevatorUse:
                        {
                            var manaFromLabel = __instance.GetManaFromLabel("Elevator Use", __instance.abilities);
                            ...
                            var synElevator = Map.Get.Elevators.Find(_ => _.Name == elevatorName);
                            var intendedResult = manaFromLabel <= __instance.Mana
                                ? Scp079EventMisc.InteractionResult.NoEnergy
                                : synElevator is null || AlphaWarheadController.Host.timeToDetonation == 0f || !synElevator.Operative || synElevator.Locked
                                    ? Scp079EventMisc.InteractionResult.Disallow
                                    : Scp079EventMisc.InteractionResult.Allow;
                                ...
                                case Scp079EventMisc.InteractionResult.Allow:
                                    {
                                        if (synElevator.Use())
                                        {
                                            __instance.Mana -= manaFromLabel;
                                            var flag3 = false;
                                            foreach (var elevator in synElevator.Lift.elevators)
                                            {
                    ...
                    case Command079.Tesla:
                        {
                            var manaFromLabel = __instance.GetManaFromLabel("Tesla Gate Burst", __instance.abilities);
                            if (__instance.CurrentRoom != null)
                            {
                                var vanillaTesla = __instance.CurrentRoom.GetComponentInChildren<TeslaGate>();
                                var synapseTesla = vanillaTesla != null ? Server.Get.Map.Teslas.Find(_ => _.Gate == vanillaTesla) : null;
                    ...
                    case Command079.Lockdown:
                        {
                            ...
                            var manaFromLabel = __instance.GetManaFromLabel("Room Lockdown", __instance.abilities);
                            GameCore.Console.AddDebugLog("SCP079", "Attempting lockdown...", MessageImportance.LeastImportant, false);

                            if (__instance.CurrentRoom != null)
                            {
                                var roomInteractablesHashSet = Scp079Interactable.InteractablesByRoomId[__instance.CurrentRoom.UniqueId];
                                var doorHashSet = new HashSet<DoorVariant>();
                                ...
                                foreach (var interactable in roomInteractablesHashSet)
                                {
                                ...
                                foreach (var scp079Interactable5 in roomInteractablesHashSet)
                                {
                                    if (!(scp079Interactable5 is null) && scp079Interactable5.TryGetComponent(out DoorVariant doorVariant3))
                                    {
                                        var doorLocked = doorVariant3.ActiveLocks == (ushort)DoorLockReason.None;
                                        if (!doorLocked)
                                        {
                                            var mode = DoorLockUtils.GetMode((DoorLockReason)doorVariant3.ActiveLocks);
                                        ...
                                        if (doorLocked)
                                        {
                                            ...
                                            _ = doorHashSet.Add(doorVariant3);
                                        }
                                    }
                                }
                                ...
                                switch (actualResult)
                                {
                                    case Scp079EventMisc.InteractionResult.Allow:
                                        {
                                            foreach (var flickerableLightController in __instance.CurrentRoom.GetComponentsInChildren<FlickerableLightController>())
                                            {
