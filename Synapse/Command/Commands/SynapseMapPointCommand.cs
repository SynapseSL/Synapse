        ...
        public CommandResult Execute(CommandContext context)
        {
            ...
            if (context.Arguments.Count == 1)
            {
                ...
                var global = new Vector3(
                    System.Single.Parse(posParts[0]),
                    System.Single.Parse(posParts[1]),
                    System.Single.Parse(posParts[2])
                );
                point = new MapPoint(context.Player.Room, global);
            }
            else
            {
                _ = Physics.Raycast(context.Player.CameraReference.transform.position, context.Player.CameraReference.transform.forward, out var raycastthit, 100f);
