...
public static class ConsumeItemPatch
{
    ...
    public static bool OnReceivedStatus(NetworkConnection conn, StatusMessage msg)
    {
        try
        {
            ...
            switch (msg.Status)
            {
                ...
                case StatusMessage.StatusType.Cancel:
                    ...
                    ev = new ConsumeItemEvent(player.Inventory.ItemInHand, ItemInteractState.Cancel, player, remainingCoolDownToCancel,
                        handler)
                    {
                        Allow = remainingCoolDownToCancel > 0 && EventManager.ExecuteEvent(ServerEventType.PlayerCancelUsingItem, player.Hub,
        ...
        {
            SynapseLogger<Synapse>.Error("Sy3 Event: Consume Item Event failed\n" + ex);
            return true;
        }
    }
        ...
        switch (ev.RadioCommand)
        {
            case RadioMessages.RadioCommand.Enable:
                if (__instance._enabled || __instance._battery <= 0f)
                    return false;
                ev.Allow = EventManager.ExecuteEvent(ServerEventType.PlayerRadioToggle, __instance.Owner, __instance,
                    true);
    ...
    public static bool MircoExecute(MicroHIDItem __instance)
    {
        ...
        switch (__instance.State)
        {
			...
			case HidState.PoweringUp:
				...
				energyToRemove = __instance._energyConsumtionCurve.Evaluate((float)(__instance._stopwatch.Elapsed.TotalSeconds / 5.95));
				break;
			...
			case HidState.Primed:
                if ((__instance.UserInput != HidUserInput.Prime && __instance._stopwatch.Elapsed.TotalSeconds >= 0.35) || __instance.RemainingEnergy <= 0f)
				{
			...
			case HidState.Firing:
				if (__instance._stopwatch.Elapsed.TotalSeconds > 1.7)
				{
					...
					__instance.Fire();
					if (__instance.RemainingEnergy == 0f || (__instance.UserInput != HidUserInput.Fire && __instance._stopwatch.Elapsed.TotalSeconds >= 2.049999952316284))
					{
